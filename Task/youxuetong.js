...
class UserInfo {
    ...
    async signIn() {
        try {
            ...
            const { signId, schoolId, id, name } = signList.data[0];
            const signUrl = `https://gwxg.xsyu.edu.cn/sign/mobile/receive/doSignByArea?id=${id}&schoolId=${schoolId}&signId=${signId}`;
            const signBody = {
                latitude: 34.09863444010416,
                longitude: 108.65533067491319,
                nationcode: "156",
                country: "中国",
                province: "陕西省",
                citycode: "156610100",
                ...
                towncode: "610118003",
                township: "五竹街道",
                streetcode: "2039016016611841091",
                street: "吕公路西段",
                inArea: 1,
...
async function getCookie() {
    if ($request && $request.method != 'OPTIONS') {
        const tokenValue = $request.headers['JWSESSION'] || $.request.headers['jwsession'];
        if (tokenValue) {
...
/*
    ...
    |    \__  \  /  ___/  _ \ /    \  /    \  \/|  |  \ /  _ \|  |  \
/\__|    |/ __ \_\___ (  <_> )   |  \ \     \___|   Y  (  <_> )  |  /
\________(____  /____  >____/|___|  /  \______  /___|  /\____/|____/ 
              \/     \/           \/          \/     \/              
*/  
/** --------------------------------辅助函数区域------------------------------------------- */
//检查变量
